...
public class Decode implements Function{
    ...
    public Result execute(String[] parameters) {
        try {
            ...
            int key = Integer.parseInt(parameters[2]) * (-1);
    ...
    private void decryptFile(String inputFilePath, int key) {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(inputFilePath));
            ...
            while ((line = reader.readLine()) != null) {
                String encryptedLine = decrypt(line, key);
                writer.write(encryptedLine);
                writer.newLine();
            }

        } catch (IOException e) {
            throw new ApplicationException("Error during file decryption", e);
        }
    }
    ...
    private Path creatingOutputFile(Path inputFilePath) {
        ...
        if (fileName.contains("[ENCRYPT]")) {
            fullName = fileName.replace("[ENCRYPT]", "[DECRYPT]");
        } else {
            fullName = fileName.replaceFirst("(\\.[^\\.]+)$", "[DECRYPT]$1");
        }

        return inputFilePath.getParent().resolve(fullName);
    }

    private String decrypt(String line, int key) {
